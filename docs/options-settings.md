...
* `gdprPersonalDataIncluded` : &lt;Boolean> Set to true if you are passing in personal information when creating the ad request to Pulse. The only location where it is possible to pass in personal information is in the `customParameters` of [ContentMetadata](http://pulse-sdks.ooyala.com/pulse-html5/latest/OO.Pulse.html#.ContentMetadata) .
